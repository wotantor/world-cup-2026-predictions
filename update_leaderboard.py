...
try:
    ...
    # Итоговый текст README.md с расширенными правилами и инструкцией
...
## ✍️ Как принять участие и вписать свое имя?
...
2. **Переименуйте ячейку:** Дважды кликните по этой ячейке и замените слово **Друг** на свое **Имя** или **Никнейм**.
   * ⚠️ *Важно:* Обязательно сохраняйте приставку **`Прогноз:`** перед вашим именем (например: `Прогноз: Стас`, `Прогноз: Даня`). Именно по этому слову робот находит вас и считает баллы!
3. **Заполняйте прогнозы:** В колонке под вашим именем пишите предполагаемый счет матчей (например, `2:1`), а соседний столбец `Баллы` заполнится автоматически, когда появятся реальные результаты матчей.
...
"""
    ...
    print("README успешно обновлен! Все изменения и инструкции внесены.")
